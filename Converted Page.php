<?php
/**
 * LottoExpert.net — AR2 Results Intelligence Page
 * Joomla 5.x + PHP 8.1+
 *
 * PURPOSE
 ...
 * - Extra Shot = column seventh, range 01..40
 */
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData('description', 'Explore ' . $stateName . ' ' . $gName . ' draw results, frequency analysis, Extra Shot statistics, and AI-powered lottery intelligence on LottoExpert.net.');
}

$jsonLdName        = isset($stateName, $gName) ? htmlspecialchars($stateName . ' ' . $gName . ' Results Intelligence', ENT_QUOTES, 'UTF-8') : 'Lottery Results Intelligence';
$jsonLdDescription = isset($stateName, $gName) ? htmlspecialchars('Frequency, recency, and full historical analysis for ' . $stateName . ' ' . $gName . ' including Extra Shot data.', ENT_QUOTES, 'UTF-8') : 'Lottery frequency and analysis page.';
$jsonLdUrl         = htmlspecialchars($canonicalNoQuery, ENT_QUOTES, 'UTF-8');
...
$doc->addCustomTag(
    ...
    '"url":"' . $jsonLdUrl . '",' .
    '"publisher":{"@type":"Organization","name":"LottoExpert.net","url":"https://lottoexpert.net"}}</script>'
);
...
/**
 * --------------------------------------------------------------------------
 * Helpers (adapted for AR2: 6 main balls 01-40, Extra Shot seventh 01-40)
 * --------------------------------------------------------------------------
 */
...
/**
 * AR2 adapter: 6 main balls (first–sixth).
 */
...
/**
 * AR2 adapter: selects seventh (Extra Shot) in addition to standard columns.
 */
...
/**
 * AR2 adapter:
 * - isBonus = false  → searches first, second, third, fourth, fifth, sixth (6 main balls)
 * - isBonus = true   → searches seventh (Extra Shot)
 */
function leGetPreviousOccurrenceDate(
    ...
    if ($isBonus) {
        $query->where($db->quoteName('seventh') . ' = ' . $db->quote($ball));
    } else {
        $query->where(
            '(' .
...
if ($input->getMethod() === 'POST' && Session::checkToken()) {
    ...
    if ($input->post->get('pfq-search', null, 'cmd') !== null) {
        $nodCurrentEs = (int) $input->post->get('pnod', $defaultWindowEs, 'int');
    }
}

$nodCurrentMain = max(10, min(700, $nodCurrentMain));
$nodCurrentEs   = max(10, min(700, $nodCurrentEs));
...
/**
 * --------------------------------------------------------------------------
 * Frequency windows — main balls (01-40) and Extra Shot (01-40)
 * --------------------------------------------------------------------------
 */
$rowsMain = leFetchRecentDraws($db, (string) $dbCol, $gameId, $nodCurrentMain);
$rowsEs   = ($nodCurrentEs === $nodCurrentMain) ? $rowsMain : leFetchRecentDraws($db, (string) $dbCol, $gameId, $nodCurrentEs);

[$mainCounts, $mainLastSeenIndex] = leInitRange(1, 40);
[$esCounts,   $esLastSeenIndex]   = leInitRange(1, 40);
...
foreach ($rowsEs as $idx => $row) {
    $ball = trim((string) ($row['seventh'] ?? ''));

    if ($ball === '' || !isset($esCounts[$ball])) {
        continue;
    }

    $esCounts[$ball]++;

    if ($esLastSeenIndex[$ball] === null) {
        $esLastSeenIndex[$ball] = (int) $idx;
    }
}
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels  = leBuildNaturalLabels(1, 40);
$mainChartValues  = [];
$mainRecencyValues = [];
...
$esChartLabels = leBuildNaturalLabels(1, 40);
$esChartValues = [];

foreach ($esChartLabels as $label) {
    $esChartValues[] = (int) ($esCounts[$label] ?? 0);
}
...
if ($drawDate !== '') {
    ...
    $prevEsDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $es, true);
    $esDrawsAgo = leGetDrawingsSinceDate($db, (string) $dbCol, $gameId, $prevEsDate, $drawDate);

    $drawHistoryRows[] = [
        'label'    => 'Extra Shot (' . lePad2($es) . ')',
        'prevDate' => $prevEsDate,
        'drawsAgo' => $esDrawsAgo,
        'isBonus'  => true,
    ];
 ...
 * --------------------------------------------------------------------------
 */
$heroInsight  = 'Latest verified draw and recent number behavior at a glance. Review the most active numbers, quiet stretches, and full historical frequency — including Extra Shot — before moving into deeper SKAI analysis.';
...
table.skai-table--bonus thead th{
  background:radial-gradient(135deg, #C73E4E 0%, #8F1F2D 100%);
}
...
<div class="skai-page">
    ...
    <div class="skai-hero-inner">
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          <div class="skai-kicker">Results Intelligence &bull; Verified Draw &bull; AR2 Extra Shot Analysis</div>
          <h1 class="skai-title">
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            ...
            <span class="skai-ball skai-ball--bonus" title="Extra Shot"><?php echo htmlspecialchars(lePad2($es), ENT_QUOTES, 'UTF-8'); ?></span>
          </div>

          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=AR2">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=AR2">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=AR2">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=AR2">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gId=<?php echo rawurlencode((string) $gId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Heatmap Analysis</a>
          </div>
        </div>
        ...
        <aside class="skai-result-panel" aria-label="Latest draw details">
          ...
          <div class="skai-meta-stack">
            ...
            <div class="skai-meta-box">
              ...
              <span class="value">Recent activity, quiet stretches, and full distribution — main numbers and Extra Shot — within your selected analysis window.</span>
            </div>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        <div class="skai-stat-value"><?php echo (int) $nodCurrentMain; ?> / <?php echo (int) $nodCurrentEs; ?></div>
        <div class="skai-stat-note">Main-number and Extra Shot draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history. All six main-number positions and the Extra Shot are included in the analysis.
        </p>
      </div>
    </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current draw easier to interpret. It shows how recently each latest number was previously seen — across all six main positions and the Extra Shot — and how the perspective shifts when you compare a short recent window with a broader historical one.
        </p>
      </div>
    </div>
      ...
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            ...
            <span class="skai-card-sub">Previous appearance date and spacing for each latest drawn value (including Extra Shot)</span>
          </div>
          <div class="skai-card-body">
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel below shows the complete main-number distribution across all values 01&ndash;40. The second panel shows the complete Extra Shot distribution for the selected bonus window. Both cover the full range 01&ndash;40.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              Full Extra Shot distribution
              <span class="skai-card-sub">All values 01&ndash;40 across the last <?php echo (int) $nodCurrentEs; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-frame">
                <canvas id="extraShotChart" aria-label="Extra Shot distribution chart" role="img"></canvas>
              </div>
            </div>
          </div>
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review of both main numbers (01&ndash;40) and Extra Shot values (01&ndash;40).
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the main-number view without losing full access to the complete dataset. The Extra Shot table covers the same 01&ndash;40 range via the seventh column.
        </p>
      </div>
    </div>
      ...
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars((string) $stateAbrev, ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars((string) $stateName, ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?>#tables">
        <div class="skai-controls-row">
          <div class="skai-controls-left">
            ...
            <label for="pnod">Extra Shot draw window</label>
            <select name="pnod" id="pnod" class="skai-select">
              <?php foreach (range(10, 700, 5) as $opt) : ?>
                <option value="<?php echo (int) $opt; ?>"<?php echo ((int) $opt === (int) $nodCurrentEs) ? ' selected="selected"' : ''; ?>>
                  <?php echo (int) $opt; ?>
                </option>
              <?php endforeach; ?>
            </select>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <!-- Main numbers table (01-40) -->
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;40 (six main-ball positions)</span>
          </div>
        ...
        <!-- Extra Shot table (01-40, column: seventh) -->
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            Extra Shot table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;40 (seventh column)</span>
          </div>

          <div class="skai-table-wrap">
            <table id="skai-es-table" class="skai-table skai-table--bonus" aria-label="Extra Shot frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
              ...
              <tbody>
                <?php for ($i = 1; $i <= 40; $i++) : ?>
                  <?php
                  $number      = ($i < 10) ? '0' . $i : (string) $i;
                  $countNumber = (int) ($esCounts[$number] ?? 0);
                  [$lastDrawSort, $lastDrawLabel] = leDrawingsAgoLabel($esLastSeenIndex[$number] ?? null, (int) $nodCurrentEs);
                  ?>
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        type="checkbox"
                        value="<?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                        aria-label="Track Extra Shot number <?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                      >
                    </td>
                  </tr>
                <?php endfor; ?>
          ...
          <div class="skai-tracked">
            <div class="skai-tracked-head">
              <h3 class="skai-tracked-title">Tracked Extra Shot values</h3>
              <div class="skai-tracked-actions">
  ...
  <section id="tools" class="skai-section" aria-labelledby="tools-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          The results page establishes context. These tools take that context into deeper modeling and structured exploration for the AR2 game.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Best next step for a broader multi-signal view. Use this after reviewing frequency and recency to move into the main SKAI intelligence workflow for AR2.
            </p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=AR2">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Use when you want a model-driven complement to the historical view shown on this page. Covers all six main-number positions and Extra Shot.
            </p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=AR2">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            <p class="skai-tool-copy">
              Useful for comparing appearance spacing and interruption behavior after reviewing current frequency data for AR2.
            </p>
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=AR2">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=AR2">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gId=<?php echo rawurlencode((string) $gId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Heatmap Analysis</a>
        <a class="skai-utility-link" href="/lottery-archives?gId=<?php echo rawurlencode((string) $gId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Lottery Archives</a>
        <a class="skai-utility-link" href="/lowest-drawn-number-analysis?gId=<?php echo rawurlencode((string) $gId); ?>&amp;stateName=<?php echo rawurlencode((string) $stateName); ?>&amp;gName=<?php echo rawurlencode((string) $gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower((string) $stateAbrev)); ?>">Lowest Number Analysis</a>
      </div>
    </div>
  </section>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing AR2 draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of the game — including all six main-number positions and the Extra Shot — easier to understand, compare, and carry into deeper SKAI analysis.
      </div>
    </div>
  </section>
...
(function () {
  ...
  var chartData = {
    ...
    esLabels: <?php echo json_encode(array_values($esChartLabels)); ?>,
    esValues: <?php echo json_encode(array_values($esChartValues)); ?>
  };
  ...
  function renderCharts() {
    ...
    var esCanvas         = document.getElementById('extraShotChart');
    ...
    if (esCanvas) {
      new Chart(esCanvas.getContext('2d'), {
        type: 'bar',
        data: {
          labels: chartData.esLabels,
          datasets: [{
            data: chartData.esValues,
            borderWidth: 0,
            borderRadius: 8,
